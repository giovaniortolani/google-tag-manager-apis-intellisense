...
interface GtmTemplateDataStorage {
  /**
...
declare module 'addMessageListener' {
     ...
     */
    function addMessageListener(messageType: string, callback: (messageType: string, message: object) => void): void;
...
declare module 'createRegex' {
   ...
   * @param flags An optional string containing flags. 'g' (global) and 'i' (ignore case) are supported.
...
declare module 'getContainerVersion' {
   ...
   */
  function getContainerVersion(): GtmContainerVersion;
...
declare module 'getEventData' {
   ...
   * @param keyPath The path of the key, where path components are separated by dots.
...
declare module 'getGoogleScript' {
     ...
     * @returns A promise that will resolve to an object containing 'script' and 'metadata' keys.
     */
    function getGoogleScript(script: 'ANALYTICS' | 'GTAG' | 'GTM', options?: { id?: string, debug?: any, timeout?: number }): GtmPromise<{ script: string, metadata: object }>;
    export = getGoogleScript;
}

declare module 'getRemoteAddress' {
    /**
     * Returns a string representation of the IP address where the request originated by reading request headers. This is a best-effort attempt and cannot guarantee accuracy.
     */
...
declare module 'getRequestHeader' {
  /**
   * Returns the value of the named request header as a string, if present, or undefined otherwise. If the header is repeated, the values are joined with ', '.
...
declare module 'getRequestQueryParameter' {
   ...
   * @returns The parameter value. Returns undefined if the parameter is not present. If repeated, the first value is returned.
   */
...
declare module 'getRequestQueryParameters' {
  /**
   * Returns the incoming HTTP request's query parameters as an object that maps query parameter names to the corresponding value or values.
   */
...
declare module 'hmacSha256' {
   ...
   */
  function hmacSha256(data: string, keyId: string, options?: { outputEncoding?: 'hex' | 'base64' | 'base64url' }): string;
  export = hmacSha256;
}
...
declare module 'parseUrl' {
   ...
   * @returns A URL object. Returns undefined for any malformed URL. For properly formatted URLs, fields not present will be an empty string or object.
   */
  function parseUrl(url: string): GtmUrl | undefined;
  export = parseUrl;
}

declare module 'returnResponse' {
  /**
   * Flushes the response that was previously set by other templates using APIs that modify the response. Defaults to HTTP 200, empty body, and no headers.
   */
...
declare module 'runContainer' {
   ...
   * @param onComplete A callback that is invoked after all the tags finish firing.
   * @param onStart A callback that is invoked immediately, before the tags start firing.
   */
  function runContainer(event: object, onComplete?: (bindToEvent: Function) => void, onStart?: (bindToEvent: Function) => void): void;
  export = runContainer;
}
...
declare module 'sendEventToGoogleAnalytics' {
     ...
     * @returns A promise that resolves to an object with a 'location' key (if present) or rejects to an object with a 'reason' key.
     */
...
declare module 'sendHttpGet' {
     ...
     * @param options Optional request options: 'headers', 'timeout' (in ms), 'authorization' object from getGoogleAuth.
     * @returns A promise that resolves to an object with 'statusCode', 'headers', and 'body'. Rejects on failure or timeout.
     */
    function sendHttpGet(url: string, options?: { headers?: { [key: string]: string }, timeout?: number, authorization?: object }): GtmPromise<{ statusCode: number, headers: object, body: string }>;
    export = sendHttpGet;
}
...
declare module 'sendHttpRequest' {
   ...
   * @param url The requested URL.
   * @param options Optional request options: 'method', 'headers', 'timeout' (in ms), 'authorization' object from getGoogleAuth.
   * @param body Optional request body.
   * @returns A promise that resolves to an object with 'statusCode', 'headers', and 'body'. Rejects on failure or timeout.
   */
  function sendHttpRequest(url: string, options?: { method?: 'GET' | 'POST' | 'PUT' | 'DELETE', headers?: { [key: string]: string }, timeout?: number, authorization?: object }, body?: string): GtmPromise<{ statusCode: number, headers: object, body: string }>;
...
declare module 'setCookie' {
   ...
   * @param options Optional cookie attributes: domain, expires, fallbackDomain, httpOnly, max-age, path, secure, and sameSite.
   * @param noEncode If true, the cookie value will not be encoded. Defaults to false.
   */
  function setCookie(name: string, value: string, options?: { domain?: string | 'auto', expires?: string, fallbackDomain?: string, httpOnly?: boolean, 'max-age'?: number, path?: string, secure?: boolean, sameSite?: 'strict' | 'lax' | 'none' }, noEncode?: boolean): void;
  export = setCookie;
}
...
declare module 'setResponseBody' {
   ...
   */
  function setResponseBody(body: string, encoding?: 'ascii' | 'utf8' | 'utf16le' | 'ucs2' | 'base64' | 'latin1' | 'binary' | 'hex'): void;
...
declare module 'sha256' {
     ...
     */
    function sha256(input: string, onSuccess: (digest: string) => void, options?: { outputEncoding?: 'base64' | 'hex' }): void;
    export = sha256;
}
...
declare module 'templateDataStorage' {
   ...
   */
  const templateDataStorage: GtmTemplateDataStorage;
...
declare module 'BigQuery' {
  ...
  const BigQuery: {
     ...
     * @returns A promise that resolves upon a successful insertion or rejects upon an error.
     */
    insert(connectionInfo: { projectId?: string, datasetId: string, tableId: string }, rows: object[], options?: { ignoreUnknownValues?: boolean, skipInvalidRows?: boolean }): GtmPromise<void>;
  };
  export = BigQuery;
}
...
declare module 'Firestore' {
    /**
     * An object that provides Firestore functions. Supports only Firestore in Native mode.
     */
    const Firestore: {
        /**
         * Reads data from a Firestore document and returns a promise.
         ...
         * @returns A promise that resolves to an object with 'id' and 'data'. Rejects if the document does not exist.
         */
        read(path: string, options?: { projectId?: string, disableCache?: boolean, transaction?: string }): GtmPromise<{ id: string, data: object }>;
        /**
         ...
         * @param options Optional request options: 'projectId', 'merge', 'transaction'.
         * @returns A promise that resolves to the ID of the document added or modified.
         */
        write(path: string, input: object, options?: { projectId?: string, merge?: boolean, transaction?: string }): GtmPromise<string>;
        /**
         * Queries the given collection and returns a promise that resolves to an array of Firestore documents.
         ...
         * @param options Optional request options: 'projectId', 'disableCache', 'limit', 'transaction'.
         * @returns A promise that resolves to an array of documents. Resolves to an empty array if no documents match.
         */
        query(collection: string, queryConditions: any[][], options?: { projectId?: string, disableCache?: boolean, limit?: number, transaction?: string }): GtmPromise<{ id: string, data: object }[]>;
        /**
         ...
         * @returns A promise that resolves to an array of document IDs for each write operation if successful, and rejects with an error if it fails.
         */
        runTransaction(callback: (transaction: string) => GtmPromise<any>, options?: { projectId?: string }): GtmPromise<string[]>;
    };
    export = Firestore;
}
...
declare module 'JSON' {
  ...
  const JSON: {
    /**
     * Parses a JSON string to construct the value or object described by the string.
     * @param stringInput The string to parse as JSON.
     * @returns The parsed object. Returns undefined if the value cannot be parsed (e.g. malformed JSON).
     */
    parse(stringInput: string): any;
    /**
     * Converts the input into a JSON string.
     * @param value The value to convert to a JSON string.
     * @returns A JSON string. Returns undefined if the value cannot be parsed (e.g. the object has a cycle).
     */
...
declare module 'Promise' {
    ...
    const Promise: {
         ...
         */
        create<T>(resolver: (resolve: (value: T | GtmPromise<T>) => void, reject: (reason?: any) => void) => void): GtmPromise<T>;
    };
    export = Promise;
}
